        ...
        <div class="card-body p-9">
            <form class="form w-100" id="kt_sign_up_form" action="registrasi-proses" method="post">
                ...
                <div class="fv-row mb-7">
                    ...
                    <input class="form-control form-control-lg form-control @error('nib') is-invalid @enderror" type="text" placeholder="Masukan Nomor NIB" id="nib" name="nib" value="{{ old('nib') }}" maxlength="13" minlength="13" autocomplete="off" required/>
                    @error('nib')
...
@push('scripts')
    <script>
        ...
        if ($('#password').val() == $('#confirmpassword').val()) {
            $('#message-match').html('Sesuai!').css('color', 'green');
        } else 
            $('#message-match').html('Password tidak sesuai!').css('color', 'red');
        });
        ...
        $("#kt_sign_up_submit").click(function(){
            if($('#password').val() == $('#confirmpassword').val()){
                ...
                event.preventDefault();
                swal("Peringatan!", "Password dan Konfirmasi Password tidak sama!", "warning");
            }
    ...
    <script>
        swal("Gagal!", "{{ session('RegistrasiError') }}", "error");
    </script>
    @endif
